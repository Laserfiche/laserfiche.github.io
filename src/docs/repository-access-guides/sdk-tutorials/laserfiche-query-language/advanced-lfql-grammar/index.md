...
LFQL grammar is similar to SQL grammar. This guide presents ordinary-language descriptions of the grammar in conjunction with railroad diagrams and formal statements in Backus-Naur form. If you are looking for a formal summary, you can focus on the Backus-Naur statements. 

## Reading Railroad Diagrams

Each railroad diagram is a representation of the Backus-Naur statement that it is paired with. The railroad diagram should be read following the path of the lines, going from left to right. Alternative paths to get from left to right represent alternative ways of forming a grammatical query. The ►►─── symbol indicates the beginning of the diagram, and the ───►◄ symbol indicates the end of the diagram.
...
![Railroad diagram showing a repetition loop](../../../assets/sha1-b6bef587a01908bd7419be433bd4a7ca108d7c73.png)

Imagine a train coming from the left, taking a "left turn" after `colspec`, then picking up another `COMMA` and another `colspec`, and so on indefinitely. In contrast, an option to take a different path without repetition looks like this:

![Railroad diagram showing a choice between OR and AND paths without repetition](../../../assets/sha1-f7b3c5c34a0de4f78c9a53651fb65e2e8b56f9e4.png)

Here, a train at the start of the diagram (the top leftmost corner) would have to choose either the `OR` or the `AND` path. Once it comes back out of the other side, the way the tracks converge makes it impossible for the train to "turn around" onto the path that it had avoided. Instead, the train has no choice but to go on "ahead" (towards the right edge of the diagram).
...
Every query takes the form of this statement. `colspec` represents the column being selected for output, and forms part of the mandatory `SELECT` clause. `fclause_opt` represents the optional `FROM` clause that specifies the tables being selected from. `wclause` is the `WHERE` clause, which specifies the conditions that the rows in the result set must meet. `oclause_opt` is the optional `ORDER BY` clause, used to sort the results. Each of these clauses is described briefly in the [introduction to constructing a LFQL query](../constructing-a-laserfiche-query-language-command/).

![Railroad diagram for the stmt grammar rule](../../../assets/sha1-8358352d07d2f466085a5194fbc19fcdae40819c.png)
...
Notice the loop that `COMMA` is in. A loop like this means that multiple colspecs, separated by `COMMA`s, can be specified, depending on how many times the loop is traversed counter-clockwise. For example, `SELECT` → `colspec` → `COMMA` → `colspec` → `COMMA` → `colspec` → `fclause_opt` → `WHERE` → `wclause` → `oclause_opt` would be a possible path.

## colspec: SELECT Statement

As indicated above, you can specify as many columns as you want as long as there is a comma between any two columns. You can also specify an `AS` option within the column specification, which lets you [assign aliases (or correlation names) to the columns](../constructing-a-laserfiche-query-language-command/#correlation-names). Aliases make it easier to refer to the columns in later clauses, such as when filtering the results by a function on the columns. The alias you assign must satisfy certain rules—see the rules for identifiers on what can count as an `IDENT`. The question mark at the end of the statement indicates that this clause is optional.

![Railroad diagram for the colspec grammar rule](../../../assets/sha1-889dc48729faa7f7b61c7408b84d0d3f8f1b0444.png)
...
The `FROM` clause specifies the tables from which to obtain results. As with specifying columns, the diagram shows that you can specify as many tables as you want, as long as each table (represented by `fromspec`) is separated from the next table by a `COMMA`. Specifying no tables is an option. This option is represented by the bottommost path, which is empty. The `LF.Entry` table will be queried if no tables are specified.

![Railroad diagram for the fclause_opt grammar rule](../../../assets/sha1-5841827d38d405615c6d31a4e1fb71df72d1c2a2.png)
...
If you choose not to assign correlation names (aliases) to tables, take the top railroad track of this diagram, which is empty. The bottom track gives you the option to assign aliases to tables, the same way you can assign aliases to columns. These aliases can then be used in other clauses to filter the results. The left `IDENT` is the table's original name, and the right `IDENT` is your chosen alias for the table. See the rules for identifiers on what can count as an `IDENT`.

![Railroad diagram for the fromspec grammar rule](../../../assets/sha1-fbfc6a7bf8fd2132c7794eafb6d5b5c81d056698.png)
...
The empty top railroad track indicates that this option can be skipped: you do not have to include an `AS` before your alias. The [table of sample queries](../../searching-repositories/examples-in-advanced-search-syntax-and-lfql/#searching-for-document-metadata) includes examples that use `AS` and examples that do not.

![Railroad diagram for the as_opt grammar rule](../../../assets/sha1-dce0aff95534a341a972e0e68ff5d647552fb729.png)
...
This is the most complicated clause, as it lets you specify conditions that the returned results must satisfy. These conditions can be based on propositional logic, pattern-matching of strings, a full-text search, or comparing ordered quantities (numbers, dates, and times). Conditions of different types can be combined using the propositional logic operators.

![Railroad diagram for the wclause grammar rule](../../../assets/sha1-ac7dee45777535d0cc6c90fa303aa4523bf717a8.png)
...
This clause lets you obtain results containing string patterns. The [table of sample queries](../../searching-repositories/examples-in-advanced-search-syntax-and-lfql/#searching-on-properties-of-annotations) includes examples using the `LIKE` operator.

![Railroad diagram for the like_opt grammar rule](../../../assets/sha1-9b12af91388ef8ed3846c076dd77193e37a71638.png)
...
![Railroad diagram for the escape_opt grammar rule](../../../assets/sha1-e61e3bb7450497ad6179e416c111f99d5415f94d.png)
...
![Railroad diagram for the between_opt grammar rule](../../../assets/sha1-cc5d8ca9e067d74fa8d0faeca5447e315d5e0aad.png)
...
![Railroad diagram for the contains_var grammar rule](../../../assets/sha1-bc223b98b4941ae69cde79a16dd190e22783ae09.png)
...
As shown above, the `WHERE` clause lets you restrict your results based on whether they satisfy certain conditions, which are made up of expressions connected by logical, arithmetical, or string manipulation operators. `exp` shows the substructure of expressions. Expressions can be made up of smaller expressions combined by another set of arithmetical and string operators. `DBLPIPE` refers to the `||` operator, which concatenates strings the same way it does in SQL. The `IDENT → LPAREN → explist_opt → RPAREN` path represents the situation where you use a [built-in function](../lfql-built-in-functions/) as an expression.

![Railroad diagram for the exp grammar rule](../../../assets/sha1-82e26acef20fdb164fff1e3df1ba8592b7936136.png)
...
![Railroad diagram for the explist_opt grammar rule](../../../assets/sha1-ae6ff94cdcb7be237b101a6978c989f8a2191d2d.png)
...
![Railroad diagram for the atom grammar rule](../../../assets/sha1-1b10e81269eac72ba6c93b3318882c3fac0734f5.png)
...
![Railroad diagram for the oclause_opt grammar rule](../../../assets/sha1-6503feb7da415990acf51571cc3743bf41746b63.png)
...
![Railroad diagram for the orderel grammar rule](../../../assets/sha1-d1179f07d519b3d1f93ddb7bdd753fab5ab4913a.png)
...
![Railroad diagram for the sortorder_opt grammar rule](../../../assets/sha1-00487e1635fd136f0c7323c63ae2248081611c94.png)
...
`<DATE>` is a date with an optional time value. A `<DATE>` literal consists of the keyword `DATE` followed by a literal string containing a date and optional time in ISO-8601 format. For example: `DATE '2007-10-22 4:45:00'`. See the list of search examples for [sample date queries](../../searching-repositories/examples-in-advanced-search-syntax-and-lfql/#date-related-searches).
