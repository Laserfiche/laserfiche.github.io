...
| Retrieve notifications<br>                        <br>Wait for up to a minute for a notification. Check that the notification is not null before getting its information. | ```<br>    Notification notif = notifManager.WaitForNotification(60*1000);<br>    if (notif != null)<br>    {<br>        ActivityRecord ar = notif.GetActivityRecord();<br>``` |
