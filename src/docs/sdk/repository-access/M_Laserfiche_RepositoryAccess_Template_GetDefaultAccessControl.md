# Template.GetDefaultAccessControl Method 
 

Returns a `TemplateSecurity` instances representing the default ACL for new templates.

**Namespace:**&nbsp;<a href="N_Laserfiche_RepositoryAccess">Laserfiche.RepositoryAccess</a><br />**Assembly:**&nbsp;Laserfiche.RepositoryAccess (in Laserfiche.RepositoryAccess.dll) Version: 12.0.0.0 (12.0.2509.107)

## Syntax

**C#**<br />
``` C#
public static TemplateSecurity GetDefaultAccessControl(
	ISession session
)
```

**VB**<br />
``` VB
Public Shared Function GetDefaultAccessControl ( 
	session As ISession
) As TemplateSecurity
```


#### Parameters
&nbsp;<dl><dt>session</dt><dd>Type: <a href="T_Laserfiche_RepositoryAccess_ISession">Laserfiche.RepositoryAccess.ISession</a><br />The Laserfiche session to use.</dd></dl>

#### Return Value
Type: <a href="T_Laserfiche_RepositoryAccess_TemplateSecurity">TemplateSecurity</a><br />A `TemplateSecurity` instances representing the default ACL for new templates.

## See Also


#### Reference
<a href="T_Laserfiche_RepositoryAccess_Template">Template Class</a><br /><a href="N_Laserfiche_RepositoryAccess">Laserfiche.RepositoryAccess Namespace</a><br />