# EntryListingBaseSettings.AddColumn Method (FieldInfo)
 

Adds a template field column to the end of the column list.

**Namespace:**&nbsp;<a href="N_Laserfiche_RepositoryAccess">Laserfiche.RepositoryAccess</a><br />**Assembly:**&nbsp;Laserfiche.RepositoryAccess (in Laserfiche.RepositoryAccess.dll) Version: 12.0.0.0 (12.0.2509.107)

## Syntax

**C#**<br />
``` C#
public bool AddColumn(
	FieldInfo fieldInfo
)
```

**VB**<br />
``` VB
Public Function AddColumn ( 
	fieldInfo As FieldInfo
) As Boolean
```


#### Parameters
&nbsp;<dl><dt>fieldInfo</dt><dd>Type: <a href="T_Laserfiche_RepositoryAccess_FieldInfo">Laserfiche.RepositoryAccess.FieldInfo</a><br />A `FieldInfo` instance which represents the template field to add a data column for.</dd></dl>

#### Return Value
Type: Boolean<br />Returns true if the column was newly added, false if the column already exists in the list.

## See Also


#### Reference
<a href="T_Laserfiche_RepositoryAccess_EntryListingBaseSettings">EntryListingBaseSettings Class</a><br /><a href="Overload_Laserfiche_RepositoryAccess_EntryListingBaseSettings_AddColumn">AddColumn Overload</a><br /><a href="N_Laserfiche_RepositoryAccess">Laserfiche.RepositoryAccess Namespace</a><br />