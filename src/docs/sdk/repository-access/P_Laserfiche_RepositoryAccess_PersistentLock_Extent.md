# PersistentLock.Extent Property 
 

Gets or sets a `LockExtent` value which represents the plock covered extent.

**Namespace:**&nbsp;<a href="N_Laserfiche_RepositoryAccess">Laserfiche.RepositoryAccess</a><br />**Assembly:**&nbsp;Laserfiche.RepositoryAccess (in Laserfiche.RepositoryAccess.dll) Version: 12.0.0.0 (12.0.2509.107)

## Syntax

**C#**<br />
``` C#
public LockExtent Extent { get; set; }
```

**VB**<br />
``` VB
Public Property Extent As LockExtent
	Get
	Set
```


#### Property Value
Type: <a href="T_Laserfiche_RepositoryAccess_LockExtent">LockExtent</a>

#### Implements
<a href="P_Laserfiche_RepositoryAccess_IPersistentLock_Extent">IPersistentLock.Extent</a><br />

## See Also


#### Reference
<a href="T_Laserfiche_RepositoryAccess_PersistentLock">PersistentLock Class</a><br /><a href="N_Laserfiche_RepositoryAccess">Laserfiche.RepositoryAccess Namespace</a><br />