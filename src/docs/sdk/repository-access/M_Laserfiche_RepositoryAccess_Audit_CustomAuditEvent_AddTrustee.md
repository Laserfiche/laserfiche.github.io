# CustomAuditEvent.AddTrustee Method 
 

Add information about a Laserfiche account to the audit record.

**Namespace:**&nbsp;<a href="N_Laserfiche_RepositoryAccess_Audit">Laserfiche.RepositoryAccess.Audit</a><br />**Assembly:**&nbsp;Laserfiche.RepositoryAccess (in Laserfiche.RepositoryAccess.dll) Version: 12.0.0.0 (12.0.2509.107)

## Syntax

**C#**<br />
``` C#
public void AddTrustee(
	AccountInfo trustee
)
```

**VB**<br />
``` VB
Public Sub AddTrustee ( 
	trustee As AccountInfo
)
```


#### Parameters
&nbsp;<dl><dt>trustee</dt><dd>Type: <a href="T_Laserfiche_RepositoryAccess_AccountInfo">Laserfiche.RepositoryAccess.AccountInfo</a><br />An `AccountInfo` instance representing the Laserfiche account whose information will be added to the audit record.</dd></dl>

## See Also


#### Reference
<a href="T_Laserfiche_RepositoryAccess_Audit_CustomAuditEvent">CustomAuditEvent Class</a><br /><a href="N_Laserfiche_RepositoryAccess_Audit">Laserfiche.RepositoryAccess.Audit Namespace</a><br />