# CloudTicket.SerializeAsString Method 
 

**Namespace:**&nbsp;<a href="N_Laserfiche_RepositoryAccess">Laserfiche.RepositoryAccess</a><br />**Assembly:**&nbsp;Laserfiche.RepositoryAccess (in Laserfiche.RepositoryAccess.dll) Version: 12.0.0.0 (12.0.2509.107)

## Syntax

**C#**<br />
``` C#
public string SerializeAsString(
	string format
)
```

**VB**<br />
``` VB
Public Function SerializeAsString ( 
	format As String
) As String
```


#### Parameters
&nbsp;<dl><dt>format</dt><dd>Type: System.String<br /></dd></dl>

#### Return Value
Type: String

#### Implements
<a href="M_Laserfiche_SecurityTokenService_ITicket_SerializeAsString">ITicket.SerializeAsString(String)</a><br />

## See Also


#### Reference
<a href="T_Laserfiche_RepositoryAccess_CloudTicket">CloudTicket Class</a><br /><a href="N_Laserfiche_RepositoryAccess">Laserfiche.RepositoryAccess Namespace</a><br />