...
1. If you are planning on using the LFDS (authorization_code) login flow
   1. Set the following values in `appsettings.json`:
      - **LFDSSTSBaseUrl**: The URI of an LFDSSTS service that has access to the LFDS server configured for your repository. This LFDSSTS service must be accessible by the API server.
...
1. Review and set any additional optional settings:
   1. In `appsettings.json`:
      ...
      - **"UploadFileSizeLimitMb"** to the desired value in **MB**.
   ...
   1. In `web.config`: Navigate to the Laserfiche API Server installation directory (by default, "C:\Program Files\Laserfiche\API Server\LFRepositoryAPI") and **open** "web.config" using a text editor as an administrator.
      - **"maxAllowedContentLength"** in "&lt;requestLimits maxAllowedContentLength="104857600" /&gt;": Set to the desired value in **bytes**.
   1. In `nlog.config`: navigate to the installation directory (e.g., "C:\Program Files\Laserfiche\API Server\LFRepositoryAPI") and open "nlog.config" using a text editor as an administrator.
   - Update default logging settings. For example, the last logging rule's "minLevel" can be increased to reduce trace logging.
   1. To allow "double-escaping" (i.e. A document name with encoded characters):
   - Navigate to the Laserfiche API Server installation directory (by default, "C:\Program Files\Laserfiche\API Server\LFRepositoryAPI") and **open** "web.config" using a text editor as an administrator. Change &lt;requestFiltering&gt;{...}&lt;/requestFiltering&gt; to &lt;requestFiltering allowDoubleEscaping="true"&gt;{...}&lt;/requestFiltering&gt;.
